...
class Database {
  constructor(config) {
    ...
    this.connection.query(`DROP TABLE IF EXISTS AuthorBooks`, (err, res) => {
      if (err) throw err;
      console.log('AuthorBooks Dropped!');
    });
    this.connection.query(`DROP TABLE IF EXISTS Authors`, (err, res) => {
      if (err) throw err;
      console.log('Authors Dropped!');
    })
    this.connection.query(`DROP TABLE IF EXISTS Books`, (err, res) => {
      if (err) throw err;
      console.log('Authors Dropped!');
    })

      // Create AuthorBooks table
    this.connection.query(`CREATE TABLE \`authorBooks\` (
      \`id\` int(11) unsigned NOT NULL AUTO_INCREMENT,
      \`authorId\` int(11) DEFAULT NULL,
      \`bookId\` int(11) DEFAULT NULL,
      PRIMARY KEY (\`id\`),
      UNIQUE KEY \`AuthorBookComposite\` (\`authorId\`,\`bookId\`)
      ) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8;`, (err, res) => {
      if (err) throw err;
      console.log('AuthorBooks Table Created');
    })

    // Load AuthorBooks Data
    this.connection.query(`INSERT INTO authorBooks (id, authorId, bookId)
      VALUES
        (1, 1, 1),
        (2, 2, 2),
        (3, 3, 2),
        ...
        (24, 18, 16);`, (err, res) => {
          if (err) throw err;
          console.log('AuthorBooks table data created');
      })
    ...
    ) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8;`, (err, res) => {
      if (err) throw err;
      console.log('Table Authors Created');
    })

    // Load Authors Data
    this.connection.query(`INSERT INTO authors (id, firstName, lastName)
      VALUES
        (15, 'Adam', 'Grant'),
        (2, 'Andrew', 'Hunt'),
        (1, 'Charles', 'Petzold'),
        ...
        (5, 'Steve', 'Krug'),
        (13, 'Steven', 'D.'),
        (17, 'W.', 'Timothy');`, (err, res) => {
        if (err) throw err;
        console.log('Authors Data Created');
      })

    // Create Books table
    this.connection.query(`CREATE TABLE books (
      id int(11) unsigned NOT NULL AUTO_INCREMENT,
    ...
    ) ENGINE=InnoDB AUTO_INCREMENT=17 DEFAULT CHARSET=utf8;`, (err, res) => {
      if (err) throw err;
      console.log('Books table created');
    })

    this.connection.query(`INSERT INTO books (id, isbn10, isbn13, title, subtitle, description, smallThumbnail, thumbnail, previewLink, infoLink, buyLink, publishedDate, pageCount, averageRating)
VALUES
